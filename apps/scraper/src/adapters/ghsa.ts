...
export const ghsaAdapter: Adapter = {
  ...
  async fetch(cursor: SourceCursor): Promise<FetchResult> {
    ...
    const items = await fetchJson<GhsaItem[]>(url, { headers });
    return { raw: items };
  },

  normalize(raw: unknown): Vuln | null {
    const r = raw as GhsaItem | null;
    if (!r || !r.ghsa_id) return null;

    const cvss = r.cvss?.score ?? undefined;
    const severity =
      cvss !== undefined && cvss > 0 ? cvssToSeverity(cvss) : deriveSeverity({ ghsaSeverity: r.severity });
    ...
    for (const a of affected) {
      ...
      if (a.package === 'symfony/symfony' || a.package.startsWith('symfony/')) tags.push('symfony');
